...
> **Maintainer note — Git authentication (2026-06-21)**
> The local clone's `origin` used to embed a personal access token directly in the
> remote URL (`https://user:ghp_...@github.com/...`), which stored the secret in
> plaintext in `.git/config`. That token has been **deleted/expired** and the URL
> has been cleaned to `https://github.com/biterik/arxiv-daily-digest.git`.
>
> When picking this project back up, **do not put tokens in the remote URL.** Use one of:
> - **SSH (preferred):** `git remote set-url origin git@github.com:biterik/arxiv-daily-digest.git`
> - **HTTPS + credential helper:** keep the clean HTTPS URL and let macOS store the
>   PAT in the keychain — `git config --global credential.helper osxkeychain`
>   (Git will prompt once and remember it).
>
> Either way the secret stays out of the repo. Note: GitHub Actions runs use the
> workflow's own secrets, not this local remote, so automated daily runs are unaffected.
